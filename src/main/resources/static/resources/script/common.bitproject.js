function getCurrentTimeDate(format){

    var today = new Date(); // getting date using date object
    var month = today.getMonth()+1; //+1 because date object give month range [0-11]
    if(month<10) month = "0"+month;
    var date = today.getDate();
    if(date<10) date = "0"+date;

    var hours = today.getHours(); // getting hours using hour object
    if(hours<10) hours = "0"+hours; //[1-24]
    var minutes = today.getMinutes();
    if(minutes<10) date = "0"+minutes;

    var currentdate = today.getFullYear() + "-" + month + "-" + date;
    var currenttime = hours +"-"+ minutes;
    var currentdatetime = today.getFullYear() + "-" + month + "-" + date  + "|" + hours + ":" + minutes;
...
function fillTable(tableid,obs,modifyFunction=null,deletefunction=null,printfunction=null){
    ...
    for(var i=0; i<obs.length; i++){
        ...
        if(hasButton){
            ...
            //print button in load table

            buttonvie.setAttribute('type','button');
            buttonvie.classList.add('btn');
            buttonvie.classList.add('btn-success');
            buttonvie.classList.add('buttonvie');
            buttonvie.innerHTML = '<i class="fa fa-eye" aria-hidden="true"></i>';
...
// this code is for filling the text box valus from the lists
...
function fillCombo3Brac(combo, message, list, attribute,attribute2, selectedvalue) {
    combo.innerHTML = "";
    if (message != "") {
        var prompt = document.createElement("option");
        prompt.value = "";
...
function fillCombo4(combo, message, list, attribute,attribute2,attribute3, selectedvalue) {
    combo.innerHTML = "";
    if (message != "") {
        var prompt = document.createElement("option");
        prompt.value = "";
    ...
    for (index in list) {
        var option = document.createElement("option");
        option.value = JSON.stringify(list[index]);
        var text = document.createTextNode(list[index][attribute] + " " + getDatafromObjectList(list[index],attribute2) + " " + getDatafromObjectList(list[index],attribute3));
        for (ind in selectedvalue){
            if (list[index][attribute] == selectedvalue[ind][attribute]) {
                option.selected = "selected";
                combo.style.border = "2px solid green";
...
function fillCombo4Brac(combo, message, list, attribute,attribute2,attribute3, selectedvalue) {
    combo.innerHTML = "";
    if (message != "") {
        var prompt = document.createElement("option");
        prompt.value = "";
    ...
    for (index in list) {
        var option = document.createElement("option");
        option.value = JSON.stringify(list[index]);
        var text = document.createTextNode(list[index][attribute] + " " + getDatafromObjectList(list[index],attribute2) + " " + "(Rs."+ getDatafromObjectList(list[index],attribute3)+ ")");
        for (ind in selectedvalue){
            if (list[index][attribute] == selectedvalue[ind][attribute]) {
                option.selected = "selected";
                combo.style.border = "2px solid green";
...
function comboBoxSelect2Binder(field,pattern,obj,prop,oldobj,parentid){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
    ...
    if (oldob != null && oldob[prop].id != ob[prop].id){
        $("#"+parentid).css('border',updated); // selectparent.children[2].style.border = updated;
    }else {
        $("#"+parentid).css('border',valid); //  selectparent.children[2].style.border = valid;
    }
}
...
function dateTimeBinder(field,pattern,obj,prop,oldobj){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob = window[field.getAttribute('data-object')];
    var oldob = window[field.getAttribute('data-oldobject')];
...
//photo choose function
function photoBinder(field,pattern,obj,prop,oldobj,imgViewPhoto){
    field.setAttribute('data-object',obj);
    field.setAttribute('data-oldobject',oldobj);
    var ob  = window[field.getAttribute('data-object')];
    var oldob  = window[field.getAttribute('data-oldobject')];
